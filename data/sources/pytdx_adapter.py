...
class PytdxAdapter:
    ...
    def _get_quote(self, market: int, code: str) -> dict:
        try:
    ...
    async def fetch_index_list(self) -> list[dict]:
        loop = asyncio.get_event_loop()
        return await loop.run_in_executor(None, self._fetch_index_list_sync)

    def _fetch_index_list_sync(self) -> list[dict]:
        try:
            from pytdx.hq import TdxHq_API
            api = TdxHq_API(heartbeat=False)
            best = self._get_best_server()
            ...
            return [{"error": True, "reason": f"PYTDX_INDEX_LIST_ERROR: {type(e).__name__}"}]

    async def fetch_stock_realtime(self, stock_code: str, market: int = 0) -> dict:
        loop = asyncio.get_event_loop()
        return await loop.run_in_executor(None, self._get_quote, market, stock_code)
